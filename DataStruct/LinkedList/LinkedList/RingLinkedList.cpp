...
void RingLinkedListMain();
SNode* CreateRingNode(SNode* pBegin, SNode* pEnd, int data);
void PrintRingLinkedList(SNode* pBegin);
SNode* InsertRingNodeData(SNode* pStart, int data, int insert);
void DeleteRingLinkedList(SNode* pStart);
...
void RingLinkedListMain()
{
	//_CrtSetBreakAlloc(96); //메모리 누수시 번호를 넣으면 할당하는 위치에 브레이크 포인트를 건다.
	...
	pEnd = CreateRingNode(pBegin, pEnd, 20);
	pEnd = CreateRingNode(pBegin, pEnd, 30);
	pEnd = CreateRingNode(pBegin, pEnd, 40);
	pEnd = CreateRingNode(pBegin, pEnd, 50);

	PrintRingLinkedList(pBegin);

	SNode* pFind = FindNodeData(pBegin, 40);
	if (pFind != NULL)
		printf("Find:%d\n", pFind->nData);

	pEnd = InsertRingNodeData(pBegin, 30, 60);//노드 삽입
	...
	DeleteRingLinkedList(pBegin); //모든노드삭제 - 이 함수를 호출하지않을시 메모리가 누수됨.
}
...
SNode* InsertRingNodeData(SNode* pStart, int data, int insert)
{
	...
	pInsert->pNext = pNode->pNext;
	pNode->pNext = pInsert;
	while (pNode->pNext != pStart) pNode = pNode->pNext;
	return pNode;
}
