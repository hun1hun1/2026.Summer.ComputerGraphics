...
SDoubleNode* CreateDoubleNode(SDoubleNode* pNode, int data);
SDoubleNode* InsertDoubleNodeData(SDoubleNode* pStart, int data, int insert);
void PrintDoubleLinkedList(SDoubleNode* pStart);
SDoubleNode* FindDoubleNodeData(SDoubleNode* pStart, int data);
void DeleteDoubleNodeData(SDoubleNode* pStart, int del);
void DeleteDoubleLinkedList(SDoubleNode* pStart);
void PrintDoubleLinkedListReverse(SDoubleNode* pEnd);
...
void DoubleLinkedListMain()
{
	...
	SDoubleNode* pBegin = NULL;
	SDoubleNode* pEnd = NULL;

	//노드 추가 테스트
	pEnd = CreateDoubleNode(pEnd, 10);
	pBegin = pEnd; //마지막 노드를 알아야 검색이 가능하므로 저장해둔다.

	pEnd = CreateDoubleNode(pEnd, 20);
	pEnd = CreateDoubleNode(pEnd, 30);
	pEnd = CreateDoubleNode(pEnd, 40);
	pEnd = CreateDoubleNode(pEnd, 50);

	PrintDoubleLinkedList(pBegin);

	SDoubleNode* pFind = FindDoubleNodeData(pBegin, 40);
	if (pFind != NULL)
		printf("Find:%d\n", pFind->nData);

	pEnd = InsertDoubleNodeData(pBegin, 30, 60);//노드 삽입

	PrintDoubleLinkedList(pBegin);

	DeleteDoubleNodeData(pBegin, 60);//노드 삭제

	PrintDoubleLinkedList(pBegin);

	PrintDoubleLinkedListReverse(pEnd);

	DeleteDoubleLinkedList(pBegin); //모든노드삭제 - 이 함수를 호출하지않을시 메모리가 누수됨.
}

SDoubleNode* CreateDoubleNode(SDoubleNode* pNode, int data)
{
	SDoubleNode* pTemp = NULL;

	pTemp = new SDoubleNode();
	pTemp->nData = data;
	pTemp->pNext = NULL;
	pTemp->pPre = NULL;
	if (pNode != NULL)
...
{
	SDoubleNode* pNode = pStart;
	SDoubleNode* pInsert = NULL;

	pNode = FindDoubleNodeData(pStart, data);

	pInsert = CreateDoubleNode(pInsert, insert);
	pInsert->pNext = pNode->pNext;
	pInsert->pNext->pPre = pInsert;
	pNode->pNext = pInsert;
	pInsert->pPre = pNode;
	while (pNode->pNext) pNode = pNode->pNext;
